...
HRESULT DXC_ddraw::iFlip()
{
	...
	if( m_bFullMode )
	{
		...
		ddVal = m_lpFrontB4->Blt( NULL, m_lpBackB4, NULL, DDBLT_WAIT, NULL ); //compatibi win7 DDFLIP_NOVSYNC
		if (G_hEditWnd != NULL) {
			if (ddVal != DDERR_SURFACELOST) m_lpDD4->FlipToGDISurface();
		}
#else

		//ddVal = m_lpBackB4flip->BltFast( 0, 0, m_lpBackB4, &m_rcFlipping, DDBLTFAST_NOCOLORKEY); // | DDBLTFAST_WAIT); Win xp
		//ddVal = m_lpFrontB4->Flip(m_lpBackB4flip, DDFLIP_WAIT);
		ddVal = m_lpBackB4flip->BltFast( 0, 0, m_lpBackB4, &m_rcFlipping, DDBLTFAST_NOCOLORKEY); // | DDBLTFAST_WAIT); Win 7
	...
	else
	{
		...
		ddVal = m_lpFrontB4->Blt(&m_rcFlipping, m_lpBackB4, NULL, DDBLT_WAIT, NULL);  //compatibi win7 DDFLIP_NOVSYNC normal xp DDBLT_WAIT
	}
