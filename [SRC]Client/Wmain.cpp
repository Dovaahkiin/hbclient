...
{	
	...
	if(cy>280) cy -= 40;
	G_hWnd = CreateWindowEx(NULL, szAppClass, "Helbreath Return", WS_POPUP, cx-320, cy-240, 
...
////////////////////////////////////////////////////////////////////////
/*void AntiSpeed() {
	EnumChildWindows(NULL,(WNDENUMPROC)NotepadFunc,NULL);
}
...
BOOL CALLBACK NotepadFunc(HWND hwnd,LPARAM lParam )
{
 char classname[256];
	...
	{
		delete G_pGame->m_pGSock;
		G_pGame->m_pGSock = NULL;
		if ((G_pGame->m_bSoundFlag) && (G_pGame->m_bMusicStat == TRUE))
		{
			if (G_pGame->m_pBGM != NULL) G_pGame->m_pBGM->bStop();
		}
		G_pGame->ChangeGameMode(DEF_GAMEMODE_ONMAINMENU);
	}
	return TRUE;
}*/
/*void Initialize(char * pCmdLine)
