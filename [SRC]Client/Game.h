...
#define DEF_SELECTEDOBJTYPE_MAGIC   3 //Drazz: Añadido para poder arrastrar magias ;)
...
public:
	...
	BOOL  m_bIconPanelTrans;
	void  DrawNewDialogBoxForIconPanel(char cType, int sX, int sY, int iFrame, BOOL bIsNoColorKey, BOOL bIsTrans);
	BOOL  bAllVsAll;
	BOOL  bStones;
	void  NotifyMsg_StonesON();
	void  NotifyMsg_StonesOFF();
	void  NotifyMsg_StoneWinner(char * pData);
	void  CityWarResult(int);
	short m_sCityWarWinner;
	short m_sAresdenDead;
	short m_sElvineDead;
	BOOL  m_bCityWar;
	void  LoadFriendList();
	void  SaveFriendList();
	...
	char  m_cFriends[13][11];
	int   m_iTotalFriends;
	int   m_iFriendIndex;
	int   m_iFriendIndex2; 
	//////////////////////////////////////////////////
	char m_cDirecionIp[16];
	int m_iPuerto;
	/////////////////////////////////////////////////
	...
	void DKGlare(int iWeaponColor, int iWeaponIndex, int *iWeaponGlare);
		void ArmorGlare(int iArmorColor, int iArmArmorIndex, int *iArmorGlare);
	void LegginsGlare(int iArmorColor, int iBodyArmorIndex, int *iArmorGlare);
	void HauberkGlare(int iArmColor, int iArmArmorIndex , int *iArmGlare);
	void HelmGlare(int iHelmColor, int iHelmIndex, int *iHelmGlare);
	void DrawDruncncity();
	...
	void DlgBoxClick_ChangeDestiny(short msX, short msY);
	void DrawDialogBox_Destiny(short msX, short msY);
...
//Snoopy end<<<<<<<<<<<<<<<<<<
	...
	DWORD m_dwTime;
	DWORD m_dwCurTime;
	DWORD m_dwCheckConnTime, m_dwCheckSprTime, m_dwCheckChatTime, m_dwCheckSpeed;
...
//	bool bCheckItemEquiped(char itemName[]); 
	bool CGame::bCheckItemEquiped(char itemName[]);// Beholder neck
...
//	int m_iAccountStatus;
	...
	char m_cGateMapName2[12];
	int  m_iGatePositX2, m_iGatePositY2;
	...
	//npc hp bar by necru
	void NotifyMsg_NpcBar(char * pData); // MORLA2- NPCBar receptores
	...
	void GoForumpage();
	void DrawDialogBox_Shop2(short msX, short msY, short msZ, char cLB);// MORLA 2.4 - Shop2
	void DlgBoxClick_Shop2(short msX, short msY); // MORLA 2.4 - paralelo
	void NotifyMsg_ItemTrade(char * pData); // Morla 2.4 - cuando trade un item por eks o dgs
//Title por E.K Code
       // int iRankEK;

	//New Addon - ZeroEoyPnk
	BOOL bZerk, bInvi, bPfm, bPfa, bShield;
	//npc hp bar by necru
	short m_sZerk, m_sInv, m_sPfm, m_sPfa, m_sShield, m_sNpcBar;
	int iNpcBar_HP, iNpcBar_MP;	

	DWORD m_dwAuraTime;

	//Added - Only GMs - ZeroEoyPnk
	struct {
		DWORD dwTime;
		char  cColor;
		char  cTxt[96];
	...
	int PartyId;
	int iPartyHp[9], iMaxPoint2[9], iBarWidth2[9], iPartySex[9], iParty_sX[9], iParty_sY[9];
	char PartyMapName[9][12];
	BOOL ActualizarParty;
	...
	//ZeroEoyPnk - NetPanel
	void DrawDialogBox_NetPanel(short msX, short msY, short msZ, char cLB);
	void DlgBoxClick_NetPanel(short msX, short msY);
	void LoadTextNetPanel(short sX, short sY, short msX, short msY, short msZ, char cLB);
	void DrawDialogBox_NetPanelConfirmation(short msX, short msY);
	void DlgBoxClick_NetPanelConfirmation(short msX, short msY);
	void ChargeTopEkList(char *pData);
	void OpenNoticement();
    ...
    int m_iSpecialBarTimer;
	int m_iReputation;

	#define BUFFER_SIZE 200

	void NotifyMsg_DGKill(char * pData); // MORLA 2.2 - Deathmach Game
	void NotifyMsg_Top15(char *pData); // MORLA 2.10 - Top 15 en deathmach
	//MORLA2.2 - Deathmach Game
	int iDGKill1, iDGKill2, iDGKill3, iDGKill4, iDGKill5, iDGKill6, iDGKill7, iDGKill8, iDGKill9, iDGKill10;//, iDGKill11, iDGKill12, iDGKill13, iDGKill14, iDGKill15 = 0;
	int iDGDeath1, iDGDeath2, iDGDeath3, iDGDeath4, iDGDeath5, iDGDeath6, iDGDeath7, iDGDeath8, iDGDeath9, iDGDeath10;//, iDGDeath11, iDGDeath12, iDGDeath13, iDGDeath14, iDGDeath15 = 0;
	char cDGName1[12], cDGName2[10], cDGName3[10], cDGName4[10], cDGName5[10], cDGName6[10], cDGName7[10], cDGName8[10], cDGName9[10], cDGName10[10];//, cDGName11[10], cDGName12[10], cDGName13[10], cDGName14[10], cDGName15[10];

	//MORLA 2.10 - Top 15 deathmach
	int iDGTOPk[16];
	int iDGTOPd[16];
	char iDGTOPn[16][15];


	void bItemDrop_Shortcut(short msX,short msY); //Drazz: Gestiona cuando arrastramos un objeto en el panel de atajos
	void bMagicDrop_Panel(short msX, short msY); //Drazz: Gestiona cuando arrastramos una magia en el panel de atajos

	BOOL _bCheckDraggingMagicRelease(short msX, short msY); //Drazz: Arrastre de magias
	BOOL bDlgBoxPress_MagicWindow(short msX, short msY);


	void DrawDialogBox_Shortcuts(short msX, short msY); //Drazz: Dibuja el panel atajos
	void DlgBoxClick_ShortCuts(short msX, short msY); //Drazz: Gestiona el click en el nuevo panel de atajos


	void CGame::ReadPanelAtajos(char cPlayerName[12]);
	void CGame::WritePanelAtajos(char cPlayerName[12]);



void CreateSystemCfg();
};
